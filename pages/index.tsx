...
const pokemonSelectedHandler = (id:number) => { selectedPokemonId === id ? setselectedPokemonId(0) :  setselectedPokemonId(id)}

const nameFilterHandler = (event:React.FormEvent<HTMLInputElement>) => {
  var filtered = event.currentTarget.value.length < 3 ? allPokemon :  allPokemon.filter(p => p.name.toLowerCase().includes((event.currentTarget.value).toLowerCase()))
  setcurrentPage(1);
...
const pageChangedHandler = ( page : number) =>{
  setcurrentPage(page);
  setShownPokemon(filteredPokemon.slice((page-1)*amountPerPage,page*amountPerPage));
  ...
  return (
    <>
    <input className='filter-input' type='text'onChange={nameFilterHandler} placeholder='Search for your Pokemon (min. 3 chars)' />
    
    {shownPokemon.length >0 ?
    <div>
      <ul className='pokemon-list'>
        {shownPokemon.map(p=><PokemonItem key={p.id} pokemon={p} selected={p.id === selectedPokemonId} pokemonSelected={pokemonSelectedHandler} />   )}
      </ul>
      <Pagination currentPage={currentPage} amountPerPage={amountPerPage} pokemonCount={filteredPokemon.length} pageChanged={pageChangedHandler} />
   </div> 
